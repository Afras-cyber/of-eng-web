...
function page() {
  const content = [
    ...
    },
    {
      ...
      subtitle: `(gemäß Anhang I Nr. 2.4.2 GefStoffV und Nummer 
        3.2 TRGS 519) – Microsoft Word`,
    },
  ];
  const CertificateComponents = ({ data }: { data: { title: string,subtitle:string } }) => {
    return (
      ...
      <Container maxWidth="md" className="p-0">
        ...
        <section>
          ...
          </Typography>
          {
            content?.map((item,j)=>{
              return (
                <CertificateComponents key={j} data={item}/>
              )
            })
          }
        </section>
